...
def plot_results(r: dict, out_path: Path, title_suffix: str = "") -> None:
    ...
    ax.set_title(
        f"100 случайных equity curves {title_suffix}", fontsize=11, weight="bold"
    )
    ax.legend()
    ...
    ax = axes[0, 1]
    ax.hist(
        r["final_equities"], bins=50, color="#10b981", alpha=0.7, edgecolor="#065f46"
    )
    ax.axvline(1.0, color="red", linewidth=2, label="Безубыток")
    ax.axvline(
        r["final_equities"].mean(),
        color="blue",
        linewidth=2,
    ...
    ax.set_title("Распределение финального результата", fontsize=11, weight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)
    ...
    ax = axes[1, 0]
    ax.hist(
        r["drawdowns"] * 100, bins=50, color="#ef4444", alpha=0.7, edgecolor="#7f1d1d"
    )
    ax.axvline(
        np.percentile(r["drawdowns"], 95) * 100,
        color="orange",
        linewidth=2,
        label=f"95% < {np.percentile(r['drawdowns'], 95) * 100:.0f}%",
    )
    ax.set_xlabel("Максимальная просадка (%)")
    ax.set_ylabel("Кол-во симуляций")
    ax.set_title("Распределение максимальной просадки", fontsize=11, weight="bold")
    ax.legend()
    ax.grid(True, alpha=0.3)

    # 4. P&L vs Drawdown scatter
    ax = axes[1, 1]
    ax.scatter(r["drawdowns"] * 100, r["final_equities"], alpha=0.3, s=10, c="#7c3aed")
    ax.axhline(1.0, color="red", linewidth=1)
    ax.set_xlabel("Макс. просадка (%)")
    ax.set_ylabel("Финальный капитал (множитель)")
    ax.set_title("P&L vs Просадка по симуляциям", fontsize=11, weight="bold")
    ax.grid(True, alpha=0.3)
...
def main() -> int:
    ...
    parser.add_argument(
        "--winrate", "-w", type=float, default=0.5, help="Win rate (0.5 = 50%%)"
    )
    parser.add_argument(
        "--rr", "-r", type=float, default=2.0, help="Risk/Reward (2.0 = 1:2)"
    )
    parser.add_argument(
        "--risk", type=float, default=0.01, help="Риск на сделку (0.01 = 1%%)"
    )
    parser.add_argument(
        "--out", type=Path, default=Path("monte-carlo.png"), help="Файл для графика"
    )
    parser.add_argument(
    ...
    print(f"Запускаю {args.simulations:,} симуляций × {args.trades} сделок каждая...")
