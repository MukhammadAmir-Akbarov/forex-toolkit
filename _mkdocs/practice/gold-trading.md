...
| Лот | Унций | Стоимость 1 пипса (пип = 0.10) | Движение цены на $1 |
|---|---|---|---|
| 0.01 (микро) | 1 | **$0.10** | $1 |
| 0.1 (мини) | 10 | **$1** | $10 |
| 1.0 (стандарт) | 100 | **$10** | $100 |
...
Из практики трейдера встречается и опасная мысль:

> *«Тилланинг характери ва ундан фойдаланган ҳолда FULL MARGIN да кириш точкалари берилади.»*
>
> (≈ «Зная характер золота, можно входить даже на полное плечо».)

!!! danger "«FULL MARGIN» / полное плечо — это путь к сливу, а не стратегия"
    Вход «на полное плечо» означает риск в десятки процентов депозита в одной
    сделке. На золоте с дневным диапазоном 150-500 пипсов **одно** движение
    против тебя обнуляет счёт. Никакое «знание характера» инструмента не
    отменяет математику риска.

    Правило проекта неизменно: **риск ≤ 1% депозита на сделку**, а размер
    позиции считается от стоп-лосса, не от «уверенности». См.
    [риск-менеджмент в учебнике](../forex-guide.md) и
    [калькулятор позиции](../tools/position-calculator.md). Цитату выше приводим
