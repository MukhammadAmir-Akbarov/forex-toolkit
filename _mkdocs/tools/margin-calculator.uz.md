...
<script>
(function() {
  // Formula tools/margin_calculator.py bilan bir xil:
  //   marja = (lotlar * kontrakt_hajmi * narx) / richak
  // Ogohlantirish chegaralari: warn > 20%, danger > 50% depozitdan
  ...
  function recalc() {
    ...
    var errors = [];
    if (!(deposit > 0))   errors.push('Balans 0 dan katta bo\'lishi kerak.');
    if (!(lots > 0))      errors.push('Lotlar 0 dan katta bo\'lishi kerak.');
    if (!(price > 0))     errors.push('Narx 0 dan katta bo\'lishi kerak.');
    if (!(leverage > 0))  errors.push('Richak 0 dan katta bo\'lishi kerak.');
    if (errors.length) {
      result.style.display = 'block';
      result.className = 'danger';
      document.getElementById('mc-headline').textContent = '—';
      warnings.innerHTML = errors.map(function(e) {
    ...
    // Python vositasi bilan bir xil chegaralar: warn > 20%, danger > 50%
    var cls = '';
    if (usagePct > 50) {
      cls = 'danger';
      warnings.innerHTML += '<div class="mc-warn mc-danger">⛔ Marja foydalanishi ' + fmtPct(usagePct) + ' — erkin marja juda kam qoldi. Bitta kuchli harakat va Margin Call bo\'lishi mumkin.</div>';
    } else if (usagePct > 20) {
      cls = 'warn';
      warnings.innerHTML += '<div class="mc-warn">⚠️ Marja foydalanishi ' + fmtPct(usagePct) + ' — hisob yuqori yuklanganiga ehtiyot bo\'ling. Pasayish uchun kam joy qoldi.</div>';
    }
    if (freeMargin < 0) {
      cls = 'danger';
      warnings.innerHTML += '<div class="mc-warn mc-danger">⛔ Erkin marja manfiy (' + fmt$(freeMargin) + ') — broker bu pozitsiyani ochishga ruxsat bermaydi.</div>';
    }
    if (cls === '') {
      warnings.innerHTML += '<div class="mc-warn" style="background:rgba(34,197,94,0.08);border-left-color:#22c55e;">✅ Marja yuklamasi normal (' + fmtPct(usagePct) + '). Pasayish uchun yetarli joy bor.</div>';
    }
