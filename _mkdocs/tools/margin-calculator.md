...
(function() {
  ...
  function recalc() {
    ...
    // Валидация
    const errors = [];
    if (!(deposit > 0))   errors.push('Депозит должен быть больше 0.');
    if (!(lots > 0))      errors.push('Лотов должно быть больше 0.');
    if (!(price > 0))     errors.push('Цена должна быть больше 0.');
    if (!(leverage > 0))  errors.push('Плечо должно быть больше 0.');
    if (errors.length) {
    ...
    document.getElementById('mc-out-lots').textContent      = lots.toFixed(2) + ' лот';
    ...
    document.getElementById('mc-out-contract').textContent  = contractSize.toLocaleString('ru-RU');
    ...
    // Пороги предупреждений (идентичны Python-версии: warn > 20%, danger > 50%)
    let cls = '';
    if (usagePct > 50) {
      cls = 'danger';
      warnings.innerHTML += `<div class="mc-warn mc-danger">⛔ Маржа ${fmtPct(usagePct)} депозита — очень мало свободной маржи. Один сильный ход против тебя — и будет Margin Call.</div>`;
    } else if (usagePct > 20) {
      cls = 'warn';
      warnings.innerHTML += `<div class="mc-warn">⚠️ Маржа ${fmtPct(usagePct)} депозита — высокая нагрузка на счёт. Мало пространства для просадки.</div>`;
    }
    if (freeMargin < 0) {
      cls = 'danger';
      warnings.innerHTML += `<div class="mc-warn mc-danger">⛔ Свободная маржа отрицательная (${fmt$(freeMargin)}) — брокер не даст открыть такую позицию.</div>`;
    }
    if (cls === '') {
      warnings.innerHTML += `<div class="mc-warn" style="background:rgba(34,197,94,0.08);border-left-color:#22c55e;">✅ Нагрузка на счёт в норме (${fmtPct(usagePct)}). Есть пространство для просадки.</div>`;
    }
    result.className = cls;
  }

  // Auto-recalc при любом изменении
