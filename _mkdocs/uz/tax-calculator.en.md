...
(function() {
  ...
  const fmtUZS = v => Math.round(v).toLocaleString('en-US') + ' UZS';
  ...
  function recalc() {
    ...
    const errors = [];
    if (!(profit >= 0)) errors.push('Profit cannot be negative.');
    if (!(loss >= 0))   errors.push('Enter the loss as a positive number.');
    if (!(rate > 0))    errors.push('Please enter the USD → UZS rate.');
    if (errors.length) {
    ...
    if (r.isLoss) {
      document.getElementById('tax-subhead').textContent = 'no tax — net loss for the year';
      warnings.innerHTML += `<div class="tax-note tax-ok">✅ Net loss for the year (${fmt$(r.net)}) — no NDFL is due. Keep your broker statement for at least 3 years in case of an audit.</div>`;
    } else {
      document.getElementById('tax-subhead').textContent = 'tax due (NDFL 12%)';
      warnings.innerHTML += `<div class="tax-note">📌 Declare your net profit by <strong>April 1, ${nextYear}</strong> in your my.soliq.uz personal account.</div>`;
      warnings.innerHTML += `<div class="tax-note">💡 You declare the annual net result (profits − losses), not each trade individually.</div>`;
    }
    warnings.innerHTML += `<div class="tax-note">⚠️ Verify the rate and procedure at soliq.uz — this is an educational estimate, not tax advice.</div>`;
  }
